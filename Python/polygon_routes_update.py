...
def insert_db_route(start_point, end_point, locations):
    route_name = "Polygon: ", start_point, " to ", end_point
    print("Polygon: ", start_point, " to ", end_point)
    sql = "UPDATE " + table_name + " SET polygon_route = '" + locations \
          + "', polygon_name = '" + ''.join(route_name) \
...
while index < len(rows):
    ...
    # adding the start point into the route json list
    start_locations = response['routes'][0]['legs'][0]['steps'][0]['start_location']
    point_list.append(start_locations)
